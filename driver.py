...
def plot_narrowband_wideband(filename):
    ...
    # print("len of mag:",len(narrow_mag_trans))
    # print("len of t_axis:",len(narrow_t_axis))
    # print("len of faxis:",len(narrow_f_axis))
    # print("shape mag:",narrow_mag_trans.shape)
    # print("shape t_axis:", narrow_t_axis.shape)
    # print("shape faxis:", narrow_f_axis.shape)
    # print(mag)
    # print(t_axis)
    # print(faxis)
    # print(mag)
...
def pca_analysis(corpus_dir):
    ...
    # list of dft stream
    dft_list = [dft_data for dft_data in dft_str_inst]
    ...
    print("Printing the variance table as a list for components:",num_com)
    ...
    # list of dft stream
    dft_list2 = [dft_data2 for dft_data2 in dft_str_inst2]
    # dft_list2 = np.array(dft_list2)
    pca_data2 = pca.PCA(dft_list2)
    # get reduced dim PCA transform data; using only enough components to capture 60% of the data.
    transform_data = pca_data2.transform(dft_list2,num_com[5])
    ...
    # getting intensity in db
    pca_trans_mag_db = 20*np.log10(pca_trans_mag)

    # computing the y axis; plotting the spectrogram with PCA comp as y axis
    y_axis = []
    for itr in range(0,len(pca_trans_mag_db)):
        y_axis.append(itr)
    y_axis = np.array(y_axis)

    # plotting the spectrogram for t vs pca component
    plt.figure()
    plt.pcolormesh(t_axis, y_axis, pca_trans_mag_db)
