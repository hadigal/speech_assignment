...
class Corpus:
    ...
    def __init__(self, transcription_dir, audio_dir):
        ...
        for directory, _subdirs, files in os.walk(self.audio):
            for f in files:
                if f.endswith(self.audioext) or f.endswith(self.audioext.upper()):
                    # Found one, get sample rate and stop looking
                    self.Fs, _ = scipy.io.wavfile.read(os.path.join(directory, f))
                    break
    ...
    def get_utterances(self, utttype="train"):
        ...
        print("TARGET DIR:{}".format(targetdir))
